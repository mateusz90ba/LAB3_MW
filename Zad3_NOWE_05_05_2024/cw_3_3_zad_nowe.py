...
print("Program do wyznaczania cech struktury dla danych trzech tekstur")
print("W widocznym oknie wybierz 3 foldery, w których zapisane są fragmenty tekstur, aby utworzyć z nich jeden plik CSV")
...
def process_texture_samples():
    ...
    input_folders = []
    for _ in range(3):
        folder = filedialog.askdirectory()  # Wybierz folder za pomocą okna dialogowego
        if folder:
            input_folders.append(folder)

    if not input_folders:
        print("Nie wybrano folderów.")
        return
    ...
    for input_folder in input_folders:
        ...
        for filename in os.listdir(input_folder):
            if filename.endswith(".png"):
                ...
                category = os.path.basename(input_folder)  # Kategoria tekstury to nazwa folderu
                data.append(features + [category])

    # Tworzenie ramki danych Pandas i zapis do jednego pliku CSV
    columns = [f"{prop}_{d}_{a}" for d in [1, 3, 5] for a in [0, 45, 90, 135] for prop in
               ['dissimilarity', 'correlation', 'contrast', 'energy', 'homogeneity', 'ASM']] + ['category']
    df = pd.DataFrame(data, columns=columns)

    # Zapis do jednego pliku CSV
    output_file = os.path.join(os.path.commonpath(input_folders), "wynik.csv")
